...
5.  **Host Building**: `nix build .#<hostname>` (shorthand) confirms that all modules merge correctly and no options conflict. This is mapped via `self.packages.x86_64-linux` in `flake.nix`.
...
| **Cachix Integration** | Prevents "No space left on device" errors by pulling pre-built binaries instead of building heavy closures on GitHub runners. |
