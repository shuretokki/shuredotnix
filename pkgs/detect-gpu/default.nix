...
pkgs.writeShellScriptBin "detect-gpu" ''
    ...
    if [ "$EUID" -ne 0 ]; then
      error "must be run as root (sudo) for accurate hardware detection"
    fi
          ...
          if echo "$current_class" | grep -Ei "VGA|3D" >/dev/null; then
            case "$current_vendor" in
              ...
              *NVIDIA*)
                ...
                elif echo "$current_device" | grep -Ei "RTX 20|GTX 16|Quadro RTX|TITAN V" >/dev/null; then
                   # Turing / Volta
                   [ $nvidia_priority -gt 4 ] && nvidia_priority=4
                elif echo "$current_device" | grep -Ei "GTX 10[0-9]{2}|TITAN Xp|NVIDIA TITAN X" >/dev/null; then
                   # Pascal
                   [ $nvidia_priority -gt 3 ] && nvidia_priority=3
                elif echo "$current_device" | grep -Ei "GTX [97][0-9]{2}|Quadro [KM]|GeForce GTX TITAN" >/dev/null; then
                   # Maxwell / Kepler
